...
const TRANSLATIONS = {
  ...
  "agent": {
    ...
    "settings": {
      ...
      "clarifying-questions": {
        "title": "允许代理人提出进一步的疑问",
        "beta-badge": "测试版",
        "description": "启用后，代理可以暂停，并向您提出简短的澄清问题，以解决您的提示可能存在歧义的情况。",
        "max-per-turn": {
          "title": "每回合可以提出的问题数量",
  ...
  "chat_window": {
    ...
    "memories": {
      "title": "回忆",
      "empty": "目前还没有任何记忆。当您与聊天机器人进行更多互动时，记忆会逐渐填充。",
      "empty_cta": "创建一个新的记忆",
      "tab_workspace": "工作空间",
      "tab_global": "全球",
      "toggle": {
        "label": "启用个性化设置",
        "description": "让你的助手能够回忆起与你或这个工作场所相关的事实，并在对话中使用这些信息。"
      },
      "auto_extraction": {
        "label": "自动回忆",
      ...
      "modal": {
        "create_title": "创造回忆",
        "edit_title": "编辑内存",
        "create_description": "记忆应该用简洁明了的语句表达。例如：“用户更喜欢使用 Python 而不是 JavaScript”。",
        "edit_description": "更新此存储内容的资料。",
        "label": "记忆",
        "placeholder": "例如，用户的姓名是 Joe，用户在 AnythingLLM 上工作，等等。",
        "create": "创造",
        "save": "保存",
        "cancel": "取消"
      }
  ...
  "model-router": {
    "title": "型号路由器",
    "description": "模型路由器允许您定义规则，根据特定条件自动将聊天消息路由到不同的LLM提供商和模型。",
    "table": {
      "name": "姓名",
      "fallback": "备用方案",
      "rules": "规则",
    ...
    "delete-confirm": "您确定要删除路由器 \"{{name}}\" 吗？\n这将删除所有其规则，并断开使用该路由器的所有工作空间。\n\n此操作是不可逆的。",
    ...
    "new-router": {
      ...
      "fallback-description": "当没有任何路由规则匹配时，也会使用。此外，还用于评估由大型语言模型（LLM）分类的规则。",
      "cooldown-label": "缓存冷却时间 (秒)",
      "cooldown-help": "路由决策在重新评估规则之前，被缓存的时间长度。将设置为 0 以禁用缓存。",
    ...
    "rules": {
      ...
      "description": "明确规定哪些聊天消息应该发送给哪些提供商和模型，以及发送的方式。",
      "add-rule": "添加规则",
      "delete-confirm": "删除规则 \"{{title}}\"?",
      "toast-delete-failed": "未能删除规则",
      "toast-reorder-failed": "未能重新应用规则",
      "no-rules": "目前还没有任何规定。",
      "empty-description": "添加一条规则，以便将聊天消息路由到特定的提供商和模型。",
      "new-rule-button": "新的规定",
      "calculated-section-label": "计算规则——按照优先级顺序进行评估",
      "llm-section-label": "LLM 规则——如果没有任何规则计算结果与给定条件匹配，则作为批量进行评估",
      "llm-rule-body": "匹配“{{description}}”后，然后将结果路由到“<route>”",
      "calculated-no-conditions": "无任何条件——前往<route>，{{route}}，</route>",
      "calculated-single-condition": "如果满足条件：<prop> {{property}}，</prop> {{comparator}}，<val>\"{{value}}\"，</val>，则将路由到 <route>{{route}}</route>",
      "calculated-multi-condition": "如果满足<cond>的条件，则将路径设置为<route>",
    ...
    "rule-form": {
      ...
      "match-description-placeholder": "例如，用户可能咨询有关法律、合同或合规等问题。",
      "match-description-help": "描述您希望该规则适用的情况。您的LLM会评估这些情况，以确定是否应该使用该规则。",
      ...
      "type-calculated-description": "根据消息的属性（如内容、标记数量或发送时间）进行匹配。",
      "type-llm-label": "大型语言模型分类",
      "type-llm-description": "使用大型语言模型（LLM），根据您提供的描述对消息进行分类。",
      ...
      "help-contains": "用逗号分隔的列表 — 如果提示包含任何这些值（不区分大小写），则匹配。",
      "help-matches": "正则表达式模式。 使用 `/pattern/flags` 选项来指定大小写敏感性（默认情况下，大小写不敏感）。",
      "bool-true": "真实",
      "bool-false": "错误"
    },
    "provider-picker": {
    ...
    "router-selection": {
      ...
      "no-routers-chat": "没有配置任何路由器。请在“设置 > AI 提供方 > 路由器”中创建一个。",
    ...
    "chat": {
      ...
      "routed-to-rule": "通过<route>、{{model}}、</route>，到达<rule>、{{ruleTitle}}、</rule>"
    }
  },
  "defaultSystemPrompt": {
    "title": null,
