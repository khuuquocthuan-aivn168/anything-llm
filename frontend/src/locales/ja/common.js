...
const TRANSLATIONS = {
  ...
  "agent": {
    ...
    "settings": {
      ...
      "clarifying-questions": {
        "title": "エージェントが、詳細を確認するための質問をしてもらうことを許可する",
        "beta-badge": "β版",
        "description": "設定が有効になっている場合、エージェントは、指示が曖昧な場合に、簡単な確認のための質問をすることができます。",
        "max-per-turn": {
          "title": "1ターンあたりの質問数",
          "description": "調査において、担当者が尋ねることができる質問の最大数はいくつですか。"
        }
      }
    }
  },
  ...
  "chat_window": {
    ...
    "memories": {
      "title": "思い出",
      "empty": "現時点では、記憶はまだありません。チャットボットとのやり取りを続けると、徐々に記憶が埋まっていくでしょう。",
      "empty_cta": "新しい記憶を作成する",
      "tab_workspace": "作業スペース",
      "tab_global": "世界的な",
      "toggle": {
        "label": "パーソナライズ機能を有効にする",
        "description": "アシスタントに、あなたやこの作業スペースに関する情報を思い出させ、会話の中で活用してもらうようにしましょう。"
      },
      "auto_extraction": {
        "label": "自動生成された思い出",
        "description": "アシスタントに、バックグラウンドで自動的に思い出を作成させるように設定してください。"
      },
      "menu": {
        "edit": "編集",
        "delete": "削除",
      ...
      "modal": {
        ...
        "edit_title": "メモリの編集",
        "create_description": "記憶は、簡潔で一文で表現されるべきです。例：「ユーザーはPythonをJavaScriptよりも好む」",
        "edit_description": "この記憶の内容を更新してください。",
        "label": "記憶",
        "placeholder": "例：ユーザー名がジョー、ユーザーが使用しているツールがAnythingLLMなど。",
        "create": "作成する",
        "save": "保存",
        "cancel": "キャンセル"
      }
    },
    "stt_unsupported": "このブラウザではマイクへのアクセスはサポートされていません。",
    "stt_mic_denied": "マイクへのアクセスができません。「許可を付与して、再度お試しください。」",
  ...
  "model-router": {
    "title": "モデルルーター",
    "description": "モデルルーターを使用すると、特定の条件に基づいて、チャットメッセージを異なるLLMプロバイダーやモデルに自動的にルーティングするためのルールを定義できます。",
    "table": {
      "name": "名前",
      "fallback": "代替案",
      "rules": "ルール",
    ...
    "empty-description": "現時点では、設定されたルーターはありません。設定を開始するために、一つ作成してください。",
    "new-router-button": "新しいルーター",
    "delete-confirm": "ルーター「{{name}}」を削除してもよろしいですか？\nこれにより、すべての設定と、それを使用しているすべてのワークスペースとの関連を解除します。\n\nこの操作は取り消すことができません。",
    ...
    "new-router": {
      ...
      "fallback-description": "以下の状況で使用されます。\n* どのルーティングルールにも一致しない場合\n* LLMによって分類されたルールを評価する場合",
      "cooldown-label": "キャッシュクールダウン (秒)",
      "cooldown-help": "ルーティングの決定が再評価されるまでの、キャッシュの保持時間を設定します。キャッシュを無効にするには、0に設定してください。",
    ...
    "rules": {
      ...
      "description": "特定のプロバイダやモデルにチャットメッセージが送信されるタイミングと方法を決定するルールを定義する。",
      "add-rule": "ルールを追加",
      ...
      "empty-description": "チャットメッセージを特定のプロバイダやモデルにルーティングを開始するためのルールを追加する。",
      "new-rule-button": "新しい規則",
      "calculated-section-label": "計算されたルール — 優先順位に基づいて、最初に評価",
      "llm-section-label": "LLMのルール—計算されたルールに一致しない場合に、まとめて評価",
      "llm-rule-body": "次に、<desc>「{{description}}」</desc> にマッチし、その後、<route>へルーティングします。",
      "calculated-no-conditions": "条件なし—ルート：<route>へ、{{route}}、</route>",
      "calculated-single-condition": "もし <prop>が条件{{property}}、</prop>が条件{{comparator}}、そして<val>が条件 \"{{value}}\"、</val>である場合、<route>へ移動する",
      "calculated-multi-condition": "もし、[{{quantifier}}]が[<cond>]である場合、[{{conditions}}]、[</cond>]を通過して、[<route>]、[{{route}}]、[</route>]へ移動する。",
    ...
    "rule-form": {
      ...
      "conditions-incomplete": "条件 {{index}} は不完全です。プロパティ、比較演算子、および値を入力してください。",
      "match-description-label": "試合の説明",
      "match-description-placeholder": "例えば、「ユーザーが、法律、契約、またはコンプライアンスに関する情報を求めている」",
      "match-description-help": "このルールが適用される状況を説明してください。LLMは、この状況に基づいて、このルールを使用すべきかどうかを判断します。",
      "route-to-label": "提供者およびモデルへのアクセス方法",
      "route-to-description": "このルールに合致する場合、このプロバイダ/モデルを使用してください。",
      "cancel": "キャンセル",
      "saving": "保存中...",
      "update-rule": "更新ルール",
      "create-rule": "ルールを作成",
      ...
      "type-calculated-description": "メッセージの内容、トークン数、または時間帯などのプロパティに基づいてマッチングを行う。",
      "type-llm-label": "LLM 分類",
      "type-llm-description": "LLM（大規模言語モデル）を使用して、提供された説明に基づいてメッセージを分類します。",
      ...
      "help-contains": "カンマ区切りのリスト—プロンプトに指定された値のいずれかが含まれている場合に一致します（大文字・小文字を区別しません）。",
      "help-matches": "正規表現パターン。大文字・小文字を区別する設定 (デフォルトは大文字・小文字を区別しない) を `/pattern/` のフラグで指定します。",
      "bool-true": "真",
      "bool-false": "誤り"
    },
    "provider-picker": {
      ...
      "setup-credentials": "{{name}} をルーティング先として使用するために、必要な認証情報を入力してください。",
      "cancel": "キャンセル",
    ...
    "router-selection": {
      ...
      "no-routers-prefix-settings": "現時点では、どのルーターも設定されていません。",
      ...
      "select-description": "この作業スペースで使用するルーターを選択してください。",
      "no-routers-chat": "ルーターは設定されていません。設定 > AI プロバイダ > モデルルーターで作成してください。",
    ...
    "chat": {
      ...
      "routed-to-rule": "<route>～</route>を経由して、<rule>～</rule>へルーティング"
    }
  },
  "defaultSystemPrompt": {
    "title": null,
